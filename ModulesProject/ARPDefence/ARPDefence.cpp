...
public:
  ArpDefender(DWORD GatewayIp, bool GatewaySpecified, DWORD IntervalSec)
      : M_GatewayIp(GatewayIp), M_IfIndex(0),
        M_GatewaySpecified(GatewaySpecified), M_IntervalSec(IntervalSec),
        M_ScanCount(0), M_AttackCount(0) {}

  bool Initialize() {
    if (M_GatewayIp == 0 || M_GatewayIp == INADDR_NONE) {
      if (!AutoDetectGateway()) {
        std::cerr
            << "[ERROR] Failed to auto-detect default gateway!\n"
    ...
    if (!ScanArpTable(FirstMac, FirstIfIndex)) {
      std::cerr << "[ERROR] Failed to obtain gateway MAC address!\n"
                << "[HINT]  Check network connectivity and ensure the gateway "
                   "is reachable\n";
      return false;
    }
    ...
    std::cout << "[*] Gateway MAC: " << M_TrustedMac
              << " [recorded as trusted]\n";
    return true;
  }
  ...
  void Run() {
    ...
    while (GRunning) {
      std::this_thread::sleep_for(std::chrono::seconds(M_IntervalSec));
      ...
      if (!ScanArpTable(CurrentMac, IfIndex)) {
        std::cout << "[" << GetTimestamp() << "] [WARN] Scan #" << M_ScanCount
                  << " failed, skipping\n";
        continue;
      }
...
private:
  ...
  bool ScanArpTable(std::string &OutMac, DWORD &OutIfIndex) const {
    ...
    for (DWORD i = 0; i < PTable->dwNumEntries; i++) {
      ...
      if (Row.dwAddr == M_GatewayIp && Row.dwPhysAddrLen > 0 &&
    ...
    for (DWORD i = 0; i < PTable->dwNumEntries; i++) {
      const MIB_IPNETROW &Row = PTable->table[i];
      if (Row.dwAddr == M_GatewayIp && Row.dwType != MIB_IPNET_TYPE_INVALID) {
        OutMac = MacToString(Row.bPhysAddr, Row.dwPhysAddrLen);
        OutIfIndex = Row.dwIndex;
        return true;
      }
  ...
  std::string TraceAttackerIp(const std::string &AttackerMac) const {
    ...
    if (Candidates.empty())
      return "Not found (attacker may have spoofed source MAC)";
  ...
  void HandleSpoofingDetection(const std::string &AttackerMac) {
    ...
    const int Choice = MessageBoxW(
        nullptr, Message.str().c_str(), L"Windows Tool - ARP Defence Alert",
...
public:
  ...
  bool Run() override {
    ...
    ArpDefender Defender(stoul(GetOption("Gateway_IP")), GatewaySpecified,
                         stoul(GetOption("Interval")));
...
extern "C" __declspec(dllexport) void StopModule() { GRunning = false; }
