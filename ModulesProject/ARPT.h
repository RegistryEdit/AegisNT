...
typedef void *(WINAPI *PFN_ArpSpoofStart)(const char *, const char *,
                                          const char *, int, int);
typedef void *(WINAPI *PFN_ArpFloodStart)(const char *, int);
typedef void *(WINAPI *PFN_ArpSniffStart)(const char *, const char *,
                                          const char *, int);
...
inline std::string WinErrorText(DWORD Code) {
  ...
  while (!Out.empty() && (Out.back() == '\r' || Out.back() == '\n'))
    Out.pop_back();
  return Out.empty() ? std::to_string(Code)
                     : (std::to_string(Code) + " (" + Out + ")");
}
...
#undef ARP_LOAD
  ...
  for (int I = 0; I < 9; I++) {
    if (!Ptrs[I]) {
      Color::PrintBad() << " GetProcAddress failed for function #" << I
                        << std::endl;
...
void MitmA(ArpDll *Dll, const char *VictimIp, const char *GatewayIp) {
  ...
  void *SniffH =
      Dll->ArpSniffStart(NULL, "tcp port 80 or tcp port 443", NULL, 1);
  if (!SniffH) {
