...
inline ProbeResult ProbeModuleInfo(const wchar_t *DllPath) {
  ...
  if (!H)
    return R;
  auto Create =
      reinterpret_cast<ModuleBase *(*)()>(GetProcAddress(H, "CreateModule"));
  auto Destroy = reinterpret_cast<void (*)(ModuleBase *)>(
      GetProcAddress(H, "DestroyModule"));
  if (!Create || !Destroy) {
    FreeLibrary(H);
    return R;
  }
  ...
  for (const auto &[Key, OptPtr] : Opts) {
    if (Idx >= 32)
      break;
    strncpy_s(R.Opts[Idx].Name, OptPtr->GetName().c_str(),
              sizeof(R.Opts[Idx].Name) - 1);
    strncpy_s(R.Opts[Idx].Val, OptPtr->GetValue().c_str(),
              sizeof(R.Opts[Idx].Val) - 1);
    strncpy_s(R.Opts[Idx].Def, OptPtr->GetDefaultValue().c_str(),
              sizeof(R.Opts[Idx].Def) - 1);
    strncpy_s(R.Opts[Idx].Desc, OptPtr->GetDescription().c_str(),
              sizeof(R.Opts[Idx].Desc) - 1);
    strncpy_s(R.Opts[Idx].Type, OptPtr->TypeName().c_str(),
              sizeof(R.Opts[Idx].Type) - 1);
...
inline bool TryLoadModuleDllFile(const wchar_t *Path, HMODULE &OutHandle,
                                 ModuleBase *&OutInst) {
...
inline void ScanModuleDirectory(const std::string &Dir,
...
inline void ScanDriverModuleDirectory(const std::string &Dir,
                                      std::vector<ModuleEntry> &Out) {
  namespace fs = std::filesystem;
  std::error_code Ec;
  if (!fs::exists(Dir, Ec))
...
inline bool LoadDriverModule(ModuleEntry &Mod) {
  ...
  if (Mod.ServiceName.empty()) {
    Mod.ServiceName = NormalizeServiceNameText(
        std::filesystem::path(Mod.Path).stem().string());
  }
  const std::wstring WPath = Utf8ToWideText(Mod.Path);
  const std::wstring WService = Utf8ToWideText(Mod.ServiceName);
  if (WPath.empty() || WService.empty())
    return false;
...
inline bool LoadLibraryModule(ModuleEntry &Mod) {
  ...
  if (!TryLoadModuleDllFile(std::filesystem::path(Mod.Path).c_str(), H, Inst))
    return false;
  Mod.Handle = H;
...
inline bool UnloadLibraryModule(ModuleEntry &Mod) {
  ...
  if (Mod.ModuleInstance) {
    auto Destroy = reinterpret_cast<void (*)(ModuleBase *)>(
        GetProcAddress(Mod.Handle, "DestroyModule"));
    if (Destroy)
      Destroy(static_cast<ModuleBase *>(Mod.ModuleInstance));
    Mod.ModuleInstance = nullptr;
  }
...
inline ModuleEntry *FindModuleByName(std::vector<ModuleEntry> &List,
...
inline void ToggleModule(std::vector<ModuleEntry> &List,
                         const std::string &Name) {
...
inline void ToggleDriver(std::vector<ModuleEntry> &List,
                         const std::string &Name) {
  auto *M = FindModuleByName(List, Name);
  if (!M)
    return;
...
inline std::vector<ModuleEntry *> FilterModuleEntries(int CategoryIndex) {
  ...
  if (CategoryIndex == 0) {
    for (auto &M : AppState::DllModules)
      if (M.Valid)
        Result.push_back(&M);
    for (auto &M : AppState::SysModules)
      if (M.Valid)
        Result.push_back(&M);
