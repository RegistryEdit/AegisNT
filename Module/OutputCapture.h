    ...
    const std::function<void(const std::string&)>& OnChunk)
{
    struct IostreamCaptureBuf final : std::streambuf {
        ...
        explicit IostreamCaptureBuf(std::function<void(const std::string&)> Callback)
    ...
    auto EnsureCrtStream = [&](FILE* Stream, const char* Name) -> bool {
        ...
        if (OnChunk) {
            OnChunk(std::string("[!] OutputCapture: failed to init ") + Name + " (fileno=" +
                    std::to_string(Fd) + ")\n");
        }
        return false;
    };
    ...
    if (!EnsureCrtStream(stdout, "stdout") || !EnsureCrtStream(stderr, "stderr")) {
