...
enum class ModuleType { Exploit, Payload, Auxiliary, Post };

inline std::string ModuleTypeToString(ModuleType Type) {
  switch (Type) {
  case ModuleType::Exploit:
    return "Exploit";
  case ModuleType::Payload:
    return "Payload";
...
public:
  ...
  void RegisterOption(const std::string &Name,
                      std::unique_ptr<Option> OptionPtr) {
  ...
  bool ValidateOptions() {
    for (const auto &Pair : Options_) {
      if (Pair.second->GetRequired() == OptionRequired::Required &&
          Pair.second->GetValue().empty())
        return false;
      if (!Pair.second->GetValue().empty() &&
          !Pair.second->Validate(Pair.second->GetValue()))
        return false;
    }
    return true;
  }
