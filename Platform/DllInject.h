...
BOOL Inject_QueueUserAPC(DWORD pid, const std::wstring& dllPath) {
		...
		if (Module32FirstW(hModSnap, &me)) {
			do {
				if (_wcsicmp(me.szExePath, dllPath.c_str()) == 0 ||
					_wcsicmp(me.szModule, dllPath.substr(dllPath.find_last_of(L"\\/") + 1).c_str()) == 0) {
					found = TRUE;
					break;
				}
		...
		do {
			if (te.th32OwnerProcessID == pid) {
				...
				if (hThread) {
					BOOL queued = FALSE;
					if (QueueUserAPC2Fn) {
						queued = QueueUserAPC2Fn((PAPCFUNC)pLoadLibrary,
							hThread, (ULONG_PTR)pRemoteMem,
							QueueUserApcFlagsSpecialUserApc);
						if (queued) specialQueued++;
					}
					if (!queued) {
						queued = QueueUserAPC((PAPCFUNC)pLoadLibrary,
							hThread, (ULONG_PTR)pRemoteMem);
						if (queued) normalQueued++;
					}
					if (!queued) {
						queueFailed++;
					}
	...
	if (specialQueued > 0 || normalQueued > 0) {
		std::cout << "[*] Waiting up to 4 seconds for APC delivery ..." << std::endl;
		for (int i = 0; i < 40; i++) {
			if (IsDllLoaded()) {
				std::cout << "[+] APC injection succeeded: DLL loaded after " << (i * 100) << " ms." << std::endl;
				if (hFallbackThread) CloseHandle(hFallbackThread);
				CloseHandle(hProcess);
				return TRUE;
			}
			Sleep(100);
		...
		std::cout << "[!] APC delivery timed out — APC queued but DLL not loaded within 4s." << std::endl;
		if (!QueueUserAPC2Fn) {
			std::cout << "[!] QueueUserAPC2 not available. Normal APC requires thread in alertable wait."
				<< std::endl;
		}
	}

	if (!hFallbackThread) {
		std::cout << "[!] No usable threads found in target — falling back to CreateRemoteThread." << std::endl;
		CloseHandle(hProcess);
		return Inject_RemoteThread(pid, dllPath);
	}

	std::cout << "[*] APC delivery not observed — falling back to thread-hijack injection on TID="
	...
	if (suspended == (DWORD)-1) {
		...
		VirtualFreeEx(hProcess, pRemoteMem, 0, MEM_RELEASE);
		CloseHandle(hProcess);
		std::cout << "[!] Thread-hijack fallback failed (cannot suspend). Try another injection method." << std::endl;
		return FALSE;
	}
	...
	if (!GetThreadContext(hFallbackThread, &ctx)) {
		...
		VirtualFreeEx(hProcess, pRemoteMem, 0, MEM_RELEASE);
		CloseHandle(hProcess);
		std::cout << "[!] GetThreadContext failed. Try another injection method." << std::endl;
		return FALSE;
	}
	...
	LPVOID pStub = VirtualAllocEx(hProcess, NULL, sizeof(stub),
	...
	SIZE_T written = 0;
	if (!WriteProcessMemory(hProcess, pStub, stub, sizeof(stub), &written) || written != sizeof(stub)) {
	...
	for (int i = 0; i < 50; i++) {
		if (IsDllLoaded()) {
			std::cout << "[+] Thread-hijack injection succeeded after " << (i * 100) << " ms." << std::endl;
			hijackSuccess = TRUE;
			break;
		}
		Sleep(100);
	...
	std::cout << "[!] Thread-hijack fallback did not load DLL within timeout." << std::endl;
	return Inject_RemoteThread(pid, dllPath);
}
