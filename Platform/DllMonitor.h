...
public:
  ...
  DllMonitor()
      : M_Pipe(INVALID_HANDLE_VALUE), M_Running(false), M_Ready(false),
  ...
  void Stop() {
    ...
    if (WasRunning) {
      ...
      HANDLE WakePipe = CreateFileW(DLL_PIPE_NAME, GENERIC_WRITE, 0, NULL,
...
private:
  ...
  void PipeLoop() {
    M_Pipe = CreateNamedPipeW(
        DLL_PIPE_NAME, PIPE_ACCESS_INBOUND,
        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 10,
        DLL_PIPE_BUFFER_SIZE, DLL_PIPE_BUFFER_SIZE, 0, NULL);
    if (M_Pipe == INVALID_HANDLE_VALUE) {
      M_CreateFailed = true;
      M_Ready = true;
      M_Running = false;
    ...
    while (M_Running) {
      ...
      while (M_Running &&
             ReadFile(M_Pipe, Buf, sizeof(Buf) - 1, &BytesRead, NULL) &&
             BytesRead > 0) {
        Buf[BytesRead] = '\0';
        if (M_Callback)
          M_Callback(ParseEvent(Buf));
      }
      DisconnectNamedPipe(M_Pipe);
    }
    CloseHandle(M_Pipe);
