...
inline std::wstring MakeFilterForExtension(const std::wstring& Extension, const std::wstring& Description = L"File") {
...
inline std::string OpenFileDialog(GLFWwindow* Window, const wchar_t* Filter = L"DLL Files\0*.dll\0Driver Files\0*.sys\0All Files\0*.*\0", const wchar_t* Title = L"Open File") {
    ...
    if (GetOpenFileNameW(&Ofn)) {
        ...
        WideCharToMultiByte(CP_UTF8, 0, FilePath, -1, &Result[0], Len, nullptr, nullptr);
...
inline std::string OpenFileDialog(GLFWwindow* Window, const std::wstring& Extension, const std::wstring& Description = L"File", const wchar_t* Title = L"Open File") {
...
inline std::string SaveFileDialog(GLFWwindow* Window,
    ...
    if (GetSaveFileNameW(&Ofn)) {
        const int Len = WideCharToMultiByte(CP_UTF8, 0, FilePath, -1, nullptr, 0, nullptr, nullptr);
        if (Len <= 0) return {};
        std::string Result(static_cast<std::size_t>(Len), '\0');
        WideCharToMultiByte(CP_UTF8, 0, FilePath, -1, &Result[0], Len, nullptr, nullptr);
        while (!Result.empty() && Result.back() == '\0') Result.pop_back();
