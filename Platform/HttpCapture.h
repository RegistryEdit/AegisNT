...
using OnHttpRequestCallback =
    std::function<void(const FlowInfo &, const HttpRequest &)>;
using OnHttpResponseCallback =
    std::function<void(const FlowInfo &, const HttpResponse &)>;
using OnTlsSniCallback =
    std::function<void(const FlowInfo &, const TlsSniInfo &)>;
using OnRawPayloadCallback =
    std::function<void(const FlowInfo &, const uint8_t *data, size_t len)>;
...
struct FlowKeyHash {
  size_t operator()(const FlowKey &k) const {
    return ((size_t)k.SrcIp << 16) ^ k.DstIp ^ ((size_t)k.SrcPort << 8) ^
           k.DstPort;
  }
};
...
private:
  ...
  struct WinDivertFuncs {
    ...
    bool (*Recv)(void *, void *, unsigned int, unsigned int *,
                 void *) = nullptr;
    bool (*HelperParsePacket)(const void *, unsigned int, void **, void **,
...
public:
  ...
  void OnPacket(uint32_t src_ip, uint32_t dst_ip, uint16_t src_port,
                uint16_t dst_port, uint32_t seq, uint32_t ack,
                const uint8_t *payload, size_t payload_len, bool is_syn,
                bool is_fin, bool is_rst, bool is_outbound);
...
private:
  ...
  uint64_t MakeKeyNormalized(uint32_t ip1, uint32_t ip2, uint16_t port1,
                             uint16_t port2) const;
};

bool ExtractSniFromClienthello(const uint8_t *data, size_t len,
                               std::string &sni_out, uint8_t &tls_version_out);

class MitmProxy {
public:
  MitmProxy(const Config &cfg, OnHttpRequestCallback req_cb,
  ...
  void OnIncomingSyn(uint32_t src_ip, uint16_t src_port, uint32_t dst_ip,
                     uint16_t dst_port);
...
bool PacketCapture::Start(const std::string &filter, OnPacketCallback cb,
  ...
  Funcs_.Recv = (bool (*)(void *, void *, unsigned int, unsigned int *,
                          void *))get("WinDivertRecv");
  Funcs_.HelperParsePacket =
      (bool (*)(const void *, unsigned int, void **, void **, unsigned char *,
                void **, void **, void **, void **, void **, unsigned int *,
                void **, unsigned int *))get("WinDivertHelperParsePacket");

  if (!Funcs_.Open || !Funcs_.Close || !Funcs_.Recv ||
      !Funcs_.HelperParsePacket) {
  ...
  HANDLE handle = (HANDLE)Funcs_.Open(filter.c_str(), 0, 0, 0x0001);
  if (!handle || handle == INVALID_HANDLE_VALUE) {
    DWORD err = GetLastError();
    std::cerr
        << "[PacketCapture] WinDivertOpen failed: " << err << "\n"
...
bool PacketCapture::IsRunning() const { return Running_.load(); }
...
void PacketCapture::CaptureLoop(const std::string &filter, OnPacketCallback cb,
  ...
  while (Running_) {
    ...
    BOOL ok =
        Funcs_.Recv(handle, buf.data(), (UINT)buf.size(), &recv_len, &addr);
    if (!ok) {
      DWORD err = GetLastError();
    ...
    Funcs_.HelperParsePacket(buf.data(), recv_len, (void **)&ip_hdr, nullptr,
                             &protocol, nullptr, nullptr, (void **)&tcp_hdr,
                             nullptr, &payload_ptr, &payload_len, &next_ptr,
                             &next_len);
...
uint64_t TcpReassembler::MakeKeyNormalized(uint32_t ip1, uint32_t ip2,
                                           uint16_t port1,
                                           uint16_t port2) const {
  uint64_t key;
...
void TcpReassembler::OnPacket(uint32_t src_ip, uint32_t dst_ip,
  ...
  if (payload_len > 0) {
    ...
    if (rel_seq == expected_rel) {
      stream.RecvBuf.insert(stream.RecvBuf.end(), payload,
                            payload + payload_len);
...
bool HttpParser::Feed(const uint8_t *data, size_t len) {
  ...
  while (remaining > 0 && State_ != ParseState::Complete &&
          ...
          if (!LineBuffer_.empty()) {
            ...
            if (IsRequest_) {
              ...
              if (space1 != std::string::npos && space2 != std::string::npos) {
                StatusCode_ = std::stoi(
                    LineBuffer_.substr(space1 + 1, space2 - space1 - 1));
      ...
      for (size_t i = 0; i < remaining; i++) {
        ...
        if (ptr[i] == '\n') {
          if (LineBuffer_.empty()) {
            ...
            State_ = ExpectedBodySize_ == 0 ? ParseState::Complete
                                            : ParseState::Body;
          } else {
    ...
    case ParseState::Body: {
      size_t to_read =
          (std::min)(remaining, ExpectedBodySize_ - BodyBytesRead_);
...
void HttpParser::ParseUrl(const std::string &url, std::string &host,
                          int &port) {
  port = 0;
  std::string tmp;
...
bool ExtractSniFromClienthello(const uint8_t *data, size_t len,
  ...
  while (offset + 4 <= len) {
    ...
    uint16_t ext_data_len =
        ((uint16_t)data[offset + 2] << 8) | data[offset + 3];
    offset += 4;
...
static X509_NAME *MakeName(const std::string &cn, const std::string &o,
                           const std::string &ou) {
...
static bool InstallCaToCurrentUserRootStore(X509 *cert) {
  ...
  PCCERT_CONTEXT existing =
      CertFindCertificateInStore(store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                                 0, CERT_FIND_EXISTING, der.data(), nullptr);
  if (existing) {
  ...
  const BOOL added = CertAddEncodedCertificateToStore(
      store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, der.data(),
      (DWORD)der.size(), CERT_STORE_ADD_REPLACE_EXISTING, nullptr);
  CertCloseStore(store, 0);
  return added == TRUE;
}

static bool LoadCaCert(const std::string &cert_path,
                       const std::string &key_path, X509 *&ca_cert,
                       EVP_PKEY *&ca_key) {
  FILE *f = nullptr;
...
static bool GenerateCertForDomain(X509 *ca_cert, EVP_PKEY *ca_key,
                                  const std::string &domain, X509 *&out_cert,
                                  EVP_PKEY *&out_key) {
  ...
  X509_EXTENSION *ext = X509V3_EXT_conf_nid(
      nullptr, nullptr, NID_subject_alt_name, (char *)san_str.c_str());
  if (ext) {
...
struct MitmProxy::Impl {
  ...
  Impl(const Config &cfg, OnHttpRequestCallback req,
  ...
  bool GenerateSelfSignedCa() {
    ...
    X509_EXTENSION *ext = X509V3_EXT_conf_nid(
        nullptr, nullptr, NID_basic_constraints, (char *)ca_str.c_str());
    if (ext) {
      X509_add_ext(cert, ext, -1);
      X509_EXTENSION_free(ext);
    }
  ...
  bool Start() {
    ...
    if (ListenSock == INVALID_SOCKET) {
      SetError("Failed to create proxy socket (WSA " +
               std::to_string(WSAGetLastError()) + ").");
      return false;
    }

    int opt = 1;
    setsockopt(ListenSock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
               sizeof(opt));
  ...
  bool ConnectUpstream(const std::string &host, uint16_t port,
    ...
    const int gai =
        getaddrinfo(host.c_str(), port_str.c_str(), &hints, &result);
    if (gai != 0 || !result) {
    ...
    if (connect(server_sock, (sockaddr *)&server_addr, sizeof(server_addr)) !=
        0) {
  ...
  void HandlePlainHttpProxy(ProxyConnection *conn, SOCKET client_sock) {
    ...
    while (Running) {
      ...
      if (need_reconnect) {
        ...
        conn->Sni = req.Host;
        conn->TargetPort = target_port;
        if (!ConnectUpstream(conn->Sni, conn->TargetPort, conn->ServerSock,
                             server_addr)) {
      ...
      request_stream << req.Method << " " << BuildOriginFormPath(req.Url)
                     << " HTTP/1.1\r\n";
      ...
      if (!SendAll(conn->ServerSock, request_bytes.data(),
                   request_bytes.size()) ||
          (!req.Body.empty() &&
           !SendAll(conn->ServerSock, req.Body.data(), req.Body.size()))) {
        break;
      }
  ...
  void HandleConnection(SOCKET client_sock) {
    ...
    uint8_t preface[16]{};
    int preface_n =
        recv(client_sock, (char *)preface, (int)sizeof(preface), MSG_PEEK);
    if (preface_n <= 0) {
    ...
    if (first == 'C' || first == 'c') {
      ...
      const uint64_t header_deadline = GetTickCount64() + 10000ULL;
      char ch = 0;
    ...
    while (GetTickCount64() < tls_deadline) {
      int available =
          recv(client_sock, (char *)tls_peek, (int)sizeof(tls_peek), MSG_PEEK);
      if (available <= 0)
        break;
      tls_len = available;
      if (ExtractSniFromClienthello(tls_peek, (size_t)tls_len, conn->Sni,
                                    tls_ver)) {
        got_sni = true;
        break;
      }
...

    {
      ...
      while (Running) {
        ...
        const bool server_pending =
            conn->ServerSsl && SSL_pending(conn->ServerSsl) > 0;
...
MitmProxy::MitmProxy(const Config &cfg, OnHttpRequestCallback req_cb,
                     OnHttpResponseCallback resp_cb)
    : Impl_(
          std::make_unique<Impl>(cfg, std::move(req_cb), std::move(resp_cb))) {}
...
struct HttpCapture::Impl {
  ...
  void OnRawPacket(RawPacket &pkt) {
    ...
    if ((pkt.SrcPort == 443 || pkt.DstPort == 443) && pkt.Payload &&
        pkt.PayloadLen > 0 && pkt.Payload[0] == 0x16 && pkt.IsOutbound) {
      std::string Sni;
      uint8_t TlsVersion = 0;
      if (ExtractSniFromClienthello(pkt.Payload, pkt.PayloadLen, Sni,
                                    TlsVersion) &&
          OnSni) {
        FlowInfo Flow;
    ...
    if (Cfg.EnableRawCallback && OnRaw) {
      ...
      fi.Protocol = (pkt.DstPort == 443 || pkt.SrcPort == 443) ? Protocol::HTTPS
                                                               : Protocol::HTTP;
      OnRaw(fi, pkt.Payload, pkt.PayloadLen);
    }

    Reassembler->OnPacket(pkt.SrcIp, pkt.DstIp, pkt.SrcPort, pkt.DstPort,
                          pkt.TcpSeq, pkt.TcpAck, pkt.Payload, pkt.PayloadLen,
                          pkt.IsSyn, pkt.IsFin, pkt.IsRst, pkt.IsOutbound);
  }
  ...
  void OnReassembled(const ReassembledData &rd) {
    ...
    if (fi.Protocol == Protocol::HTTPS) {
      if (rd.Dir == FlowDir::ClientToServer && !rd.Data.empty()) {
        ...
        uint8_t tls_ver = 0;
        if (rd.Data[0] == 0x16 &&
            ExtractSniFromClienthello(rd.Data.data(), rd.Data.size(), sni,
                                      tls_ver)) {
          fi.Sni = sni;
          TlsSniInfo sni_info;
          ...
          if (OnSni)
            OnSni(fi, sni_info);
          std::cout << "[SNI] " << IpToString(fi.SrcIp) << ":" << fi.SrcPort
                    << " -> " << sni << "\n";
        }
      }
      if (!Cfg.HttpsMitm)
...
bool HttpCapture::Start() {
  ...
  auto self = Impl_.get();
  bool ok = Impl_->Capture.Start(
      filter, [self](RawPacket &pkt) { self->OnRawPacket(pkt); }, Impl_->Cfg);
...
bool HttpCapture::IsRunning() const { return Impl_->Capture.IsRunning(); }
...
int main() {
  ...
  capture.OnTlsSni([](const http_capture::FlowInfo &fi,
    ...
    std::cout << "[SNI] " << IpToStr(fi.SrcIp) << ":" << fi.SrcPort << " -> "
              << sni.ServerName << " (TLS v" << (int)sni.TlsVersion << ")\n";
  });
