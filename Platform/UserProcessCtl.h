...
typedef BOOL(WINAPI *T_EndTask)(HWND hWnd, BOOL fShutDown, BOOL fForce);

T_EndTask pfnEndTask = nullptr;

void PatchThread(DWORD Ptid) {
  static auto exitAddr = (DWORDLONG)GetProcAddress(
      GetModuleHandleW(L"kernel32.dll"), "ExitProcess");
...
bool NtTerminate(DWORD Pid) {
  ...
  if (hProcess) {
    PNtTerminateProcess NtTerminateProcess =
        (PNtTerminateProcess)GetProcAddress(GetModuleHandleW(L"ntdll.dll"),
                                            "NtTerminateProcess");
...
bool InjectSuicide(HANDLE hProc) {
  ...
  pack.adExitProcess =
      (intptr_t)GetProcAddress(GetModuleHandle(L"KERNEL32.dll"), "ExitProcess");
  if (!pack.adExitProcess) {
    printf("GetProcAddress Failed\n");
    return false;
  }
  LPVOID lpData =
      VirtualAllocEx(hProc, NULL, sizeof pack, MEM_COMMIT, PAGE_READWRITE);
  if (!lpData) {
  ...
  LPVOID lpCode = VirtualAllocEx(hProc, NULL, dwFunSize, MEM_COMMIT,
                                 PAGE_EXECUTE_READWRITE);
  if (!lpCode) {
    printf("VirtualAllocEx 2 Failed\n");
    return false;
  }
  ret = WriteProcessMemory(hProc, lpCode, (LPVOID)&RemoteThreadProc_Suicide,
                           dwFunSize, &useless);
  if (!ret) {
  ...
  HANDLE hThread = CreateRemoteThread(
      hProc, NULL, 0, (LPTHREAD_START_ROUTINE)lpCode, lpData, 0, NULL);
  if (!hThread) {
