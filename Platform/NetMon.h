...
struct PidKey {
  ...
  bool operator==(const PidKey &Other) const {
    return LocalPort == Other.LocalPort && RemotePort == Other.RemotePort &&
           Protocol == Other.Protocol && IsIpv6 == Other.IsIpv6;
  }
};
...
public:
  uint32_t Lookup(uint32_t *InOutLocal, uint32_t *InOutRemote,
...

    {
      ...
      if (It != Cache_.end() &&
          (Now - It->second.Timestamp) < NETMON_CACHE_TTL_MS) {

        std::memcpy(InOutLocal, It->second.CorrectedLocal, IsIpv6 ? 16 : 4);
        std::memcpy(InOutRemote, It->second.CorrectedRemote, IsIpv6 ? 16 : 4);
    ...
    if (Protocol == IPPROTO_TCP) {
      ...
      uint32_t Pid = QueryTcpTable(
          InOutLocal, InOutRemote, *InOutLocalPort, *InOutRemotePort, IsIpv6,
          InOutLocal, InOutRemote, InOutLocalPort, InOutRemotePort);
      return Pid;
    }
    ...
    if (Protocol == IPPROTO_UDP) {
      E.Pid = QueryUdpTable(InOutLocal, *InOutLocalPort, IsIpv6);
    }
...
private:
  ...
  static uint32_t QueryTcpTable(const uint32_t *PktLocal,
                                const uint32_t *PktRemote,
                                uint16_t PktLocalPort, uint16_t PktRemotePort,
                                bool IsIpv6, uint32_t *OutLocal,
                                uint32_t *OutRemote, uint16_t *OutLocalPort,
      ...
      if (Size > 0) {
        ...
        if (::GetExtendedTcpTable(Table, &Size, FALSE, AF_INET,
          ...
          for (DWORD I = 0; I < Table->dwNumEntries; I++) {
            ...
            if (Row.dwLocalPort == LocalNbo && Row.dwRemotePort == RemoteNbo) {
          ...
          for (DWORD I = 0; I < Table->dwNumEntries; I++) {
            auto &Row = Table->table[I];
            if (Row.dwLocalPort == LocalNbo && Row.dwRemotePort == 0 &&
  ...
  static uint32_t QueryUdpTable(const uint32_t *PktLocal, uint16_t PktLocalPort,
                                bool IsIpv6) {

    if (!IsIpv6) {
      DWORD Size = 0;
      ::GetExtendedUdpTable(nullptr, &Size, FALSE, AF_INET, UDP_TABLE_OWNER_PID,
                            0);
      if (Size > 0) {
        std::vector<uint8_t> Buf(Size);
    ...
    DWORD Size = 0;
    ::GetExtendedUdpTable(nullptr, &Size, FALSE, AF_INET6, UDP_TABLE_OWNER_PID,
                          0);
    if (Size == 0)
      return 0;
...
inline bool ParsePacket(const uint8_t *Data, unsigned int Len,
  ...
  if (Version == 4) {
    ...
    if (Pkt->Protocol == IPPROTO_TCP) {
      ...
      uint8_t DataOff =
          static_cast<uint8_t>(((DataOffsetFlags >> 12) & 0x0F) * 4);
...
inline void GetProcessName(uint32_t Pid, char *OutBuf, size_t OutSize) {
  OutBuf[0] = '\0';
  if (Pid == 0) {
    std::strcpy(OutBuf, "Idle");
    return;
  }
  if (Pid == 4) {
    std::strcpy(OutBuf, "System");
    return;
  }
  ...
  if (::QueryFullProcessImageNameW(H, 0, Path, &PathLen)) {
    ...
    if (Len > 0 && (size_t)Len < OutSize)
      ::WideCharToMultiByte(CP_UTF8, 0, P, -1, OutBuf, (int)OutSize, nullptr,
                            nullptr);
  }
  ::CloseHandle(H);
...
inline std::string IpToString(const uint32_t *Addr, bool IsIpv6) {
  ...
  return ::InetNtopA(Family, Addr, Buf, (DWORD)sizeof(Buf)) ? std::string(Buf)
                                                            : std::string("?");
}
...
inline bool NetMon_IsWinDivertDriverRunning() {
  ...
  for (const wchar_t *Name : Names) {
    ...
    DWORD Error = Queried && Status.dwCurrentState == SERVICE_RUNNING
                      ? ERROR_SUCCESS
                      : (Queried ? ERROR_SERVICE_NOT_ACTIVE : ::GetLastError());
...
inline DWORD WINAPI NetMon_NetworkThread(LPVOID) {
  if (!NetMon_IsWinDivertDriverRunning()) {
    ::fprintf(stderr,
              "[NetMon] WinDivert driver service is not running (error=%u)\n",
              ::GetLastError());
  ...
  HANDLE Handle = Divert.Open("ip and (tcp or udp)", WINDIVERT_LAYER_NETWORK, 0,
  ...
  while (GetRunningFlag()) {
    ...
    Pkt.Pid =
        GetPidCache().Lookup(Pkt.LocalAddr, Pkt.RemoteAddr, &Pkt.LocalPort,
                             &Pkt.RemotePort, Pkt.Protocol, Pkt.IsIpv6);
...
inline bool NetMon_Start(NetMon_Callback Callback) {
  ...
  HANDLE HNet =
      ::CreateThread(nullptr, 0, NetMon_NetworkThread, nullptr, 0, nullptr);
  if (!HNet) {
    Running = false;
...
inline bool NetMon_IsRunning() { return GetRunningFlag(); }
