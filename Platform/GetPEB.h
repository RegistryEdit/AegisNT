...
typedef NTSTATUS(NTAPI *pfnNtQueryInformationProcess)(HANDLE, PROCESSINFOCLASS,
                                                      PVOID, ULONG, PULONG);
...
public:
  ...
  static BOOL ReadPebInfoText(DWORD Pid, std::string &Output) {
    ...
    auto Query = HNtdll
                     ...
                     : nullptr;
    if (!Query || !NT_SUCCESS(Query(HProcess, ProcessBasicInformation, &Pbi,
                                    sizeof(Pbi), nullptr))) {
    ...
    Query(HProcess, static_cast<PROCESSINFOCLASS>(26), &Wow64PebAddress,
          sizeof(Wow64PebAddress), nullptr);
    if (Wow64PebAddress != 0) {
      ToolPeb32 Peb32{};
      if (!ReadProcessMemory(HProcess,
                             reinterpret_cast<LPCVOID>(Wow64PebAddress), &Peb32,
      ...
      Stream << "PEB / Process Information\n"
             ...
             << "WOW64 PEB Address: 0x" << std::hex << Wow64PebAddress
             << std::dec << "\n"
             << "ImageBaseAddress: 0x" << std::hex << Peb32.ImageBaseAddress
             << std::dec << "\n"
             << "Ldr: 0x" << std::hex << Peb32.Ldr << std::dec << "\n"
             << "ProcessParameters: 0x" << std::hex << Peb32.ProcessParameters
             << std::dec << "\n"
             << "BeingDebugged: " << (Peb32.BeingDebugged ? "Yes" : "No")
             << "\n"
             << "BitField: 0x" << std::hex
             << static_cast<unsigned>(Peb32.BitField) << std::dec << "\n"
             << "  ImageUsesLargePages: "
             << ((Peb32.BitField & 0x01) ? "Yes" : "No") << "\n"
             << "  IsProtectedProcess: "
             << ((Peb32.BitField & 0x02) ? "Yes" : "No") << "\n"
             << "  IsImageDynamicallyRelocated: "
             << ((Peb32.BitField & 0x04) ? "Yes" : "No") << "\n"
             << "  IsPackagedProcess: "
             << ((Peb32.BitField & 0x10) ? "Yes" : "No") << "\n"
             << "  IsAppContainer: " << ((Peb32.BitField & 0x20) ? "Yes" : "No")
             << "\n"
             << "  IsProtectedProcessLight: "
             << ((Peb32.BitField & 0x40) ? "Yes" : "No") << "\n"
             << "  IsLongPathAware: "
             << ((Peb32.BitField & 0x80) ? "Yes" : "No") << "\n";

      ToolProcessParameters32 Params32{};
      if (Peb32.ProcessParameters != 0 &&
          ReadProcessMemory(HProcess,
                            reinterpret_cast<LPCVOID>(static_cast<ULONG_PTR>(
                                Peb32.ProcessParameters)),
                            &Params32, sizeof(Params32), nullptr)) {
        Stream << "\nProcess Parameters (32-bit)\n"
               << "---------------------------\n"
               << "Length: " << Params32.Length << " / "
               << Params32.MaximumLength << " bytes\n"
               << "Flags: 0x" << std::hex << Params32.Flags << std::dec << "\n"
               << "ConsoleHandle: 0x" << std::hex << Params32.ConsoleHandle
               << std::dec << "\n"
               << "StandardInput: 0x" << std::hex << Params32.StandardInput
               << std::dec << "\n"
               << "StandardOutput: 0x" << std::hex << Params32.StandardOutput
               << std::dec << "\n"
               << "StandardError: 0x" << std::hex << Params32.StandardError
               << std::dec << "\n"
               << "Environment: 0x" << std::hex << Params32.Environment
               << std::dec << "\n"
               << "ImagePath: "
               << WideToUtf8(ReadUnicode32(Params32.ImagePathName)) << "\n"
               << "CommandLine: "
               << WideToUtf8(ReadUnicode32(Params32.CommandLine)) << "\n"
               << "CurrentDirectory: "
               << WideToUtf8(ReadUnicode32(Params32.CurrentDirectory.DosPath))
               << "\n"
               << "DllPath: " << WideToUtf8(ReadUnicode32(Params32.DllPath))
               << "\n";
      } else {
        Stream << "\n32-bit Process Parameters: unavailable (Error "
               << GetLastError() << ")\n";
      }
      CloseHandle(HProcess);
      Output = Stream.str();
    ...
    auto ReadUnicode = [&](const UNICODE_STRING &Value) {
      ...
      std::wstring Text(Value.Length / sizeof(WCHAR), L'\0');
      SIZE_T Read = 0;
      if (!ReadProcessMemory(HProcess, Value.Buffer, Text.data(), Value.Length,
                             &Read) ||
          Read != Value.Length)
        return std::wstring{};
      return Text;
    ...
    const char *Architecture =
        ProcessMachine == IMAGE_FILE_MACHINE_I386   ? "x86 (WOW64)"
        : NativeMachine == IMAGE_FILE_MACHINE_AMD64 ? "x64"
        : NativeMachine == IMAGE_FILE_MACHINE_ARM64 ? "ARM64"
                                                    : "Unknown";
    ...
    Stream << "PEB / Process Information\n"
           ...
           << "Priority Class: 0x" << std::hex << GetPriorityClass(HProcess)
           << std::dec << "\n"
    ...
    if (Peb.ProcessParameters &&
      ...
      const std::wstring CurrentDirectory =
          ReadUnicode(Params.CurrentDirectory.DosPath);
      const std::wstring DllPath = ReadUnicode(Params.DllPath);
      Stream << "\nProcess Parameters\n"
             << "------------------\n"
             << "Length: " << Params.Length << " / " << Params.MaximumLength
             << " bytes\n"
  ...
  static BOOL ReadModuleListText(DWORD Pid, std::string &Output) {
    ...
    if (Query && Wow64PebAddress != 0) {
      ...
      while (Current != 0 && Current != RemoteHead && Count < 4096) {
        ...
        const std::string FullPath =
            WideToUtf8(ReadUnicode32(Entry.FullDllName));
        Stream << "[" << Count << "] " << (Name.empty() ? "(unnamed)" : Name)
               << "\n"
               << "  Full Path: "
               << (FullPath.empty() ? "(unavailable)" : FullPath) << "\n"
    ...
    ToolPeb Peb{};
    if (!Query ||
        !NT_SUCCESS(Query(HProcess, ProcessBasicInformation, &Pbi, sizeof(Pbi),
                          nullptr)) ||
        !ReadProcessMemory(HProcess, Pbi.PebBaseAddress, &Peb, sizeof(Peb),
                           nullptr) ||
        !Peb.Ldr) {
      CloseHandle(HProcess);
      return FALSE;
    ...
    const auto RemoteHead = reinterpret_cast<LIST_ENTRY *>(
        reinterpret_cast<BYTE *>(Peb.Ldr) +
        offsetof(ToolPebLdrData, InLoadOrderModuleList));
    ...
    while (Current && Current != RemoteHead && Count < 4096) {
      ...
      SIZE_T BytesRead = 0;
      if (!ReadProcessMemory(HProcess, Current, &Entry, sizeof(Entry),
                             &BytesRead) ||
          BytesRead != sizeof(Entry)) {
        Stream << "\nTraversal stopped at " << Current << " (Error "
               << GetLastError() << ")\n";
        break;
      }
      const std::string Name = WideToUtf8(ReadRemoteUnicode(Entry.BaseDllName));
      const std::string FullPath =
          WideToUtf8(ReadRemoteUnicode(Entry.FullDllName));
      Stream << "[" << Count << "] " << (Name.empty() ? "(unnamed)" : Name)
             << "\n"
             << "  Full Path: "
             << (FullPath.empty() ? "(unavailable)" : FullPath) << "\n"
    ...
    if (Count == 0) {
      Stream << "PEB loader list returned no entries; using Toolhelp "
                "fallback.\n\n";
  ...
  static BOOL ReadProcessParameters(HANDLE HProcess,
    ...
    return ReadProcessMemory(HProcess, Peb.ProcessParameters, &Params,
                             sizeof(Params), nullptr) != FALSE;
  }
  ...
  static BOOL ReadCommandLine(DWORD Pid, WCHAR *Buffer, DWORD BufferSize) {
    ...
    RTL_USER_PROCESS_PARAMETERS Params{};
    if (!ReadProcessParameters(Pid, Params))
      return FALSE;
    ...
    BOOL Result = ReadProcessMemory(HProcess, Params.CommandLine.Buffer, Buffer,
                                    ReadSize, nullptr);
    if (Result)
  ...
  static BOOL ReadImagePath(DWORD Pid, WCHAR *Buffer, DWORD BufferSize) {
    ...
    SIZE_T ReadSize =
        min(Params.ImagePathName.Length, (USHORT)(BufferSize - 2));
  ...
  static BOOL PrintPebInfo(DWORD Pid) {
    ...
    if (ReadProcessMemory(HProcess, Peb.ProcessParameters, &Params,
      ...
      if (ReadProcessMemory(HProcess, Params.CommandLine.Buffer, Buffer,
                            ReadSize, nullptr)) {
        Buffer[ReadSize / sizeof(WCHAR)] = L'\0';
      ...
      if (ReadProcessMemory(HProcess, Params.ImagePathName.Buffer, Buffer,
                            ReadSize, nullptr)) {
        Buffer[ReadSize / sizeof(WCHAR)] = L'\0';
        printf("ImagePath:          %ws\n", Buffer);
      }
    ...
    if (ReadProcessMemory(HProcess, Peb.Ldr, &Ldr, sizeof(Ldr), nullptr)) {
      ...
      while (Current != Head) {
        ...
        if (Entry.DllBase && Entry.BaseDllName.Buffer) {
          SIZE_T NameSize =
              min(Entry.BaseDllName.Length, (USHORT)(sizeof(DllName) - 2));
        ...
        printf("  %-30ws base=%p size=0x%X\n",
               DllName[0] ? DllName : L"(unknown)", Entry.DllBase,
               Entry.SizeOfImage);
