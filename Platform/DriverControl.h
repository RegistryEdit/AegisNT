...
bool GetFullDriverPathValue(const wchar_t* InputPath, wchar_t* OutPath, DWORD Size) {
...
bool QueryServiceStatusValue(SC_HANDLE Service, SERVICE_STATUS_PROCESS* Status) {
    DWORD BytesNeeded = 0;
...
bool QueryDriverServiceState(const wchar_t* ServiceName, DWORD* State) {
    ...
    ScHandleGuard Service(OpenServiceW(Scm.Get(), ServiceName, SERVICE_QUERY_STATUS));
    if (!Service.Valid()) {
...
bool IsDriverServiceRunning(const wchar_t* ServiceName) {
    ...
    return QueryDriverServiceState(ServiceName, &State) && State == SERVICE_RUNNING;
}
...
bool WaitForServiceDeletion(SC_HANDLE Scm, const wchar_t* ServiceName, DWORD TimeoutMs) {
    const ULONGLONG Deadline = GetTickCount64() + TimeoutMs;
    do {
        ScHandleGuard Existing(OpenServiceW(Scm, ServiceName, SERVICE_QUERY_STATUS));
        if (!Existing.Valid()) {
            DWORD Error = GetLastError();
            if (Error == ERROR_SERVICE_DOES_NOT_EXIST) {
                return true;
...
DWORD StopAndDeleteDriverService(SC_HANDLE Scm, const wchar_t* ServiceName) {
    ...
    if (QueryServiceStatusValue(Existing.Get(), &Status)) {
        if (Status.dwCurrentState != SERVICE_STOPPED && Status.dwCurrentState != SERVICE_STOP_PENDING) {
            ...
            if (!ControlService(Existing.Get(), SERVICE_CONTROL_STOP, &ServiceStatus)) {
                DWORD Error = GetLastError();
...
DWORD OpenOrCreateDriverService(SC_HANDLE Scm, const wchar_t* ServiceName, const wchar_t* FullPath, ScHandleGuard* Service) {
    if (!Scm || !ServiceName || !*ServiceName || !FullPath || !*FullPath || !Service) {
        SetLastError(ERROR_INVALID_PARAMETER);
        return 1;
    }
    ...
    for (;;) {
        ...
        if (OpenError != ERROR_SERVICE_DOES_NOT_EXIST && OpenError != ERROR_SERVICE_MARKED_FOR_DELETE) {
        ...
        if (CreateError == ERROR_SERVICE_EXISTS || CreateError == ERROR_DUP_NAME || CreateError == ERROR_SERVICE_MARKED_FOR_DELETE) {
            if (CreateError == ERROR_SERVICE_MARKED_FOR_DELETE && !WaitForServiceDeletion(Scm, ServiceName, 10000)) {
                PrintLastErrorMessage(L"Timed out waiting for driver service deletion");
                return 1;
            }
            continue;
...
DWORD LoadDriverService(const wchar_t* DriverPath, const wchar_t* ServiceName) {
    ...
    if (OpenOrCreateDriverService(Scm.Get(), ServiceName, FullPath, &Service) != 0) {
        return 1;
    }
    ...
    if (!QueryServiceStatusValue(Service.Get(), &Status) || Status.dwCurrentState != SERVICE_RUNNING) {
    ...
    if (!WaitForDriverDeviceReady(ServiceName, 15000)) {
        DWORD State = SERVICE_STOPPED;
        if (!QueryDriverServiceState(ServiceName, &State) || State != SERVICE_RUNNING) {
            PrintLastErrorMessage(L"Timed out waiting for driver device to become ready");
            return 1;
        }
    }
