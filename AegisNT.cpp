...
class WindowsToolWindow final : public QWidget
{
  ...
  private:
    ...
    QWidget *CreateSidebar()
    {
        ...
        for (int Index = 0; Index < static_cast<int>(AegisNT::PageDefinitions().size()); ++Index)
        {
            if (Index == 6)
            {
    ...
    {
        if (Index < 0 || Index >= static_cast<int>(AegisNT::PageDefinitions().size()))
            return;
        const bool WasLoaded = PageLoaded[Index];
        EnsurePageLoaded(Index);
        NavigationPanelWidget->setCurrentItem(QString::number(Index));
        TitleLabelWidget->setProperty("LanguageOriginal_text", AegisNT::PageDefinitions()[Index].Title);
        SubtitleLabelWidget->setProperty("LanguageOriginal_text", AegisNT::PageDefinitions()[Index].Subtitle);
