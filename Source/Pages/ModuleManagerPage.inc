QWidget *CreateModuleManagerPage() {
  ...
  Paths->setText(
      "Modules: " + ConfigurationPaths("Modules", "./Modules").join("; ") +
  ...
  const auto SelectedIsDriver = [Table] {
    const int Row = Table->currentRow();
    return Row >= 0 && Table->item(Row, 0) &&
           Table->item(Row, 0)->data(Qt::UserRole + 1).toBool();
  };
  ...
  const auto Populate = [Kind, Search, Table, RefreshActions] {
    ...
    Table->clearContents();
    Table->setRowCount(0);
    const auto AppendModule = [Query, Table](ModuleEntry &Module,
                                             bool IsDriver) {
      ...
      if (!Query.isEmpty() && !Name.contains(Query, Qt::CaseInsensitive) &&
          ...
          !Category.contains(Query, Qt::CaseInsensitive))
        return;
      ...
      Table->setItem(Row, 3,
                     new QTableWidgetItem(
                         Module.Valid ? (Module.Loaded ? "Active" : "Inactive")
                                      : "Skipped"));
  ...
  QObject::connect(Table, &QTableWidget::itemSelectionChanged, Page,
                   RefreshActions);
  QObject::connect(Refresh, &QPushButton::clicked, Page, [Page, Populate] {
    if (ModuleRunning.load()) {
      ShowWarningNotice(
          Page, "ModuleManager",
          "Wait for the running module to finish before reloading modules.");
      return;
    }
    ScanRuntimeModules();
    Populate();
  ...
  QObject::connect(
      AddModule, &QPushButton::clicked, Page, [Page, Kind, Populate] {
        const QString Path = QFileDialog::getOpenFileName(
            Page, "Select DLL", QString(), "DLL files (*.dll)");
        if (Path.isEmpty())
          return;
        const QString AbsolutePath = QFileInfo(Path).absoluteFilePath();
        const bool Exists = std::any_of(
            DllModules.begin(), DllModules.end(),
            [&AbsolutePath](const ModuleEntry &Module) {
              return QString::fromStdString(Module.Path)
                         .compare(AbsolutePath, Qt::CaseInsensitive) == 0;
            });
        if (!Exists) {
        ...
        Kind->setCurrentIndex(0);
        Populate();
        ShowSuccessNotice(Page, "ModuleManager",
                          Exists ? "Module is already in the list."
                                 : "DLL module added.");
      });
  QObject::connect(
      Remove, &QPushButton::clicked, Page,
      [Page, SelectedPath, SelectedIsDriver, Populate] {
        ...
        const auto Entry = std::find_if(
            List.begin(), List.end(), [&Path](const ModuleEntry &Module) {
        ...
        Populate();
        ShowSuccessNotice(Page, "ModuleManager",
                          "Item removed from the module list.");
      });
  QObject::connect(
      ...
      [Page, Table, Populate](const QPoint &Position) {
        ...
        auto *ToggleAction =
            new QAction(Entry->Loaded ? "Unload" : "Load", Menu);
        ToggleAction->setEnabled(Entry->Valid);
        Menu->addAction(ToggleAction);
        ConnectMenuAction(ToggleAction, Page, [Page, Path, IsDriver, Populate] {
          std::vector<ModuleEntry> &TargetList =
              IsDriver ? DriverModules : DllModules;
          const auto Target =
              std::find_if(TargetList.begin(), TargetList.end(),
                           [&Path](const ModuleEntry &Module) {
                             return QString::fromStdString(Module.Path) == Path;
                           });
          ...
          if (!Ok)
            ShowErrorNotice(Page, "ModuleManager",
                            Error.isEmpty() ? "The operation failed." : Error);
          else
            ShowSuccessNotice(Page, "ModuleManager",
                              IsDriver ? "Driver state updated."
